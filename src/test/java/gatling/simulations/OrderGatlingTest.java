...
public class OrderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all orders").get("/services/provaprogetto/api/orders").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new order")
                        .post("/services/provaprogetto/api/orders")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"orderDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"totalAmount\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created order").get("/services/provaprogetto${new_order_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created order").delete("/services/provaprogetto${new_order_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
